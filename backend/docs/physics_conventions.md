...
## Legacy NC simulator mode

The `/simulate_nc_legacy` and `/veff_nc_legacy` endpoints reproduce the older
Streamlit simulator as a compatibility mode. This mode uses `u = 1/r` and the
polynomial massive-particle potential:
...
```text
V(r) = [1 - (4/pi)(atan(r/sqrt(theta)) - r sqrt(theta)/(r^2 + theta))] u^2 - 2u^3
```

Massive legacy orbits are generated by the old quadrature over `u` using the
allowed region `E - V(r) > 0`. Photon legacy orbits use `1/b^2 - V(r) > 0`.
When `rst` is omitted, the backend applies the old UI table `rst = s/L` for
massive particles and `rst = 50` for photons.
